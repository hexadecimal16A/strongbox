        ...
        stage('Building')
        {
            steps {
                withMavenPlus(timestamps: true, mavenLocalRepo: workspace().getM2LocalRepoPath(), mavenSettingsConfig: '67aaee2b-ca74-4ae1-8eb9-c8f16eb5e534')
                {
                    ...
                    sh "mvn -U clean install -T2C -Dintegration.tests -Dprepare.revision -Dmaven.test.failure.ignore=true -Pcoverage -Dtest=ArtifactEntryServiceTest#testParallelSaveTheSameArtifactToDifferentRepositories  -DfailIfNoTests=false"
                }
            }
        }
        stage('Code Analysis') {
