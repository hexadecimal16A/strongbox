        ...
        stage('Building')
        {
            steps {
                withMavenPlus(timestamps: true, mavenLocalRepo: workspace().getM2LocalRepoPath(), mavenSettingsConfig: '67aaee2b-ca74-4ae1-8eb9-c8f16eb5e534')
                {
                    ...
                    sh "mvn -U clean install -T2C -pl strongbox-storage/strongbox-storage-api -Dtest=ArtifactEntryServiceTest#testParallelSaveTheSameArtifactToDifferentRepositories"
                }
            }
        }
        stage('Code Analysis') {
